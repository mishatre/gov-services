...
export function getEnvConfig<S extends z.ZodObject, D extends Partial<z.infer<S>>>(
    schema: S,
    prefix: string,
    additionalPrefix?: string,
    defaultConfig?: D,
): Omit<z.infer<S>, keyof D> & { [K in keyof D]-?: NonNullable<D[K]> } {
    // WithDefaults<z.infer<S>, D> {
    ...
    for (const key of keys as Array<keyof z.infer<S>>) {
        data[key] =
            getEnv(toSnakeCase(String(key)).toUpperCase(), prefix, additionalPrefix) ??
            defaultConfig?.[key]
    }
